 ...
 * 
 * @package  NewsCore WP
 * @author   Epic Web Solutions
 * @link     http://epicwebsol.com/
 *
 */
 ...
 if ( !class_exists( 'Radium_Theme_Importer' ) ) {
	...
	class Radium_Theme_Importer {
		 ...
		 * @var object
		 */
		public $theme_options_framework = 'et_divi_options'; //supports radium framework and option tree
	    ...
	    public function add_admin() {
	        ...
	        add_submenu_page('et_divi_options', "Import Demo Data", "Import Demo Data", 'switch_themes', 'on_click_demo_installer', array($this, 'demo_installer'));
    	...
    	public function intro_html() {
			...
			<div style="background-color: #ffffff; margin:0px 0; padding: 5px 10px 10px; color: #444; border: 2px solid #6c2eb9; clear:both; width:90%; line-height:18px;">
			    <p class="tie_message_hint">Importing demo data (posts, pages, images, theme settings, etc.) is the easiest way to setup your theme. It will
			    allow you to quickly edit everything instead of creating content from scratch. When you import the data, the following things will happen:</p>
			      ...
			      <ul style="padding-left: 20px;list-style-position: inside;list-style-type: square;}">
			          ...
			          <li>Images will be downloaded from our server. Unless otherwise noted, these images are copyrighted and are for demo use only!</li>
			          <li>Please click import button once and do not refresh the page. Wait until the process is complete - it can take up to several minutes depending on your hosting provider and server speed.</li>
			          <li>If for some reason the importer times out or fails to import successfully, you can click the import button again. Doing so could result in duplicate sample content, which can be easily deleted.</li>
			          <li>Questions? Issues? <a target="_blank" href="http://help.besuperfly.com">Visit our support portal or submit a ticket</a> and we'll be happy to help you.</li>
			      </ul>
			 </div>

			 <div style="background-color: #FFFFFF; margin:10px 0; padding: 5px 10px 10px;color: #444; border: 2px solid #6c2eb9; clear:both; width:90%; line-height:18px;"><p class="tie_message_hint">Before you begin, make sure all the required plugins are activated.</p></div>

			 <?php
echo the_permalink();
			 if( !empty($this->imported_demos) ) { ?>

			  	<div style="background-color: #ffffff; margin:10px 0; padding: 5px 10px; color: #444; border: 2px solid #6c2eb9; clear:both; width:90%; line-height:18px;">
	    ...
	    public function demo_installer() {
	        ...
	        ?><div id="icon-tools" class="icon32"><br></div>
				<div class="import-demo-content-spacing"><br></div>
	        <h2><span style="color: #ffffff; background:#6c2eb9; padding: 20px 20px 15px 20px; border-radius: 5px 5px 0px 0px; margin-bottom:0px;">Import Child Theme Data</span></h2>
	...
	<style class="import-form-styles">
		.radium-import-start {
	    ...
	    public function set_demo_theme_options( $file ) {
			...
			if ( file_exists( $file ) ) {
				...
				$data = unserialize($data) or die("Failed to unserialize data."); //Should never see this unless the content is broken
                    if ( $data ) {
