    ...
    class TGM_Plugin_Activation {
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' == $_GET['tgmpa-install'] ) ) {
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . admin_url('admin.php?page=radium_demo_installer') . '" title="' . __( 'f to the Dashboard', 'tgmpa' ) . '">' . __( 'f to the Dashboard', 'tgmpa' ) . '</a>' ) . '</p>';
    ...
    class TGMPA_List_Table extends WP_List_Table {
        ...
        protected function _gather_plugin_data() {
            ...
            foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                ...
                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    ...
                    $table_data[$i]['status'] = sprintf( '%1$s', __( '<b style="color:red;">Installed But Not Activated</b>', 'tgmpa' ) );
                }
        ...
        public function column_cb( $item ) {
			...
			if(isset($_GET['checked'])) {
				if(strcmp($item['sanitized_plugin'], urldecode($_GET['checked']))==0)  {
					$checked = 'checked';
				}
			}
            ...
            return sprintf( '<input '.$checked.' type="checkbox" name="%1$s[]" value="%2$s" id="%3$s" />', $this->_args['singular'], $value, $item['sanitized_plugin'] );
        ...
        public function no_items() {
            ...
            printf( __( 'No plugins to install or activate. <a href="%1$s" title="Go To Import Demo Data Page">Go To Import Demo Data Page</a>', 'tgmpa' ),  admin_url('admin.php?page=on_click_demo_installer')  );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
    ...
    if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
        ...
        class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
            ...
            public function bulk_footer() {
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . admin_url('admin.php?page=on_click_demo_installer') . '" title="' . __( 'Go To Import Demo Data Page', 'tgmpa' ) . '">' . __( 'Go To Import Demo Data Page', 'tgmpa' ) . '</a>' ) . '</p>';
                    echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
